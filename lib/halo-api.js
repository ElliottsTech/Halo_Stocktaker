            ...
            if (itemData.isSerialised && hasSerialisedAssets) {
              try {
                ...
                if (assets.assets && assets.assets.length > 0) {
                  // Debug: log first asset's keys to identify cost field name
                  if (!this._assetCostFieldLogged) {
                    const firstAsset = assets.assets[0];
                    const costKeys = Object.keys(firstAsset).filter(k => /cost|price|value|purchase/i.test(k));
                    console.log(`🔍 Asset cost-related fields available: ${costKeys.length ? costKeys.join(', ') : 'NONE FOUND'}`);
                    console.log(`🔍 Full first asset keys: ${Object.keys(firstAsset).join(', ')}`);
                    this._assetCostFieldLogged = true;
                  }

                  locationData.serialNumbers = assets.assets.map(asset => ({
                    id: asset.id,
