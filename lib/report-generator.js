...
function generateReportPDF(stocktake) {
  return new Promise((resolve, reject) => {
    ...
    const missing = (report.missingSerials || []).map(m => ({
      ...m,
      itemName: stripEmoji(m.itemName),
      locationName: stripEmoji(m.locationName)
    }));
    if (missing.length > 0) {
      const totalSerials = missing.reduce((s, m) => s + (m.serialNumbers || []).length, 0);
      y = ensureSpace(doc, y, 24);
      doc.font('Helvetica-Bold').fontSize(11).fillColor('#000')
        .text(`Missing Serials (${missing.length} items, ${totalSerials} serials)`, MARGIN, y);
      y += 16;
      doc.font('Helvetica').fontSize(8.5).fillColor('#c53030');
      for (const m of missing) {
        const text = `${m.itemName}  —  ${m.locationName}:  ${(m.serialNumbers || []).join(', ')}`;
